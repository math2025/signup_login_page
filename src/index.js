...
app.post("/signup/faculty", async (req, res) => {
  ...
  if (existingUser) {
    return res.render("signup_faculty", {
...
app.post("/signup/parents", async (req, res) => {
  ...
  const existingUser = await collection.findOne({ email: email });
  if (existingUser) {
    return res.render("signup_parents", {
      error: "Email already registered. Try logging in.",
    });
  }
...
app.post("/signup/students", async (req, res) => {
  ...
  const existingUser = await collection.findOne({ email: email });
  if (existingUser) {
    return res.render("signup_students", {
      error: "Email already registered. Try logging in.",
    });
  }
...
app.post("/login/faculty", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.render("login_faculty", {
        error: "No user found with the given name and email.",
      });
    }
...
app.post("/login/parents", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.render("login_parents", {
        error: "No user found with the given name and email.",
      });
    }
...
app.post("/login/students", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.render("login_students", {
        error: "No user found with the given name and email.",
      });
    }
